import Vue from 'vue';
import { isRef, ref, computed, watch, inject, provide, onUnmounted, watchEffect } from '@vue/composition-api';
import { isEqual, cloneDeep, each, get, isEmpty, last, defaults } from 'lodash-es';
...
function useForm({
  ...
  const handleValidate = () => execValidate(cloneDeep(activeValues.value), form);
...
function useField(fieldPath, {
  ...
  // value
  const value = computed(() => get(form.values.value, fieldPath, initialState.value));
  ...
  // touched
  const touched = computed(() => get(form.touched.value, fieldPath, initialState.touched));
  ...
  // error
  const error = computed(() => get(form.errors.value, fieldPath, initialState.error));
  ...
  // active
  const active = computed(() => get(form.actives.value, fieldPath, initialState.active));
  const setActive = (
    active,
  ...
  // editable
  const editable = computed(() => get(form.editable.value, fieldPath, initialState.editable));
  ...
  // visible
  const visible = computed(() => get(form.visible.value, fieldPath, initialState.visible));
...
export { useField, useFieldArray, useForm, useFormInject, useFormProvide };
